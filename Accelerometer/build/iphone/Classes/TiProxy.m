...
//Rare exceptions to indicate a bug in the _accelerometer code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_accelerometer = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_accelerometer]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.accelerometer.module"
